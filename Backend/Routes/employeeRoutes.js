...
router.post("/login",async(req,res)=>{
    ...
    try{
        ...
        {
            const token= jwt.sign({Name:dataquery[0][0].Name, Email:dataquery[0][0].Email,Role:dataquery[0][0].Role},process.env.SECRET_KEY);
            // obj.token=token;
...
try{
    const data=await mySqlPool.query(`INSERT INTO employee_info (Name,Email,Password,Role) VALUES (?,?,?,?)`,[obj.Name, obj.Email, obj.Password,role]);
    console.log(data);
    if(!data)
    {
    ...
    const token= jwt.sign({Name:urlData.name, Email:urlData.email,Role:"Admin"},process.env.SECRET_KEY);
    obj.token=token;
...
const dataMiddle = async (req, res, next) => {
    try {
        ...
        const [findData] = await mySqlPool.query('SELECT * FROM employee_info WHERE Email = ?', [req.body.email]);
...
try{
    ...
    const data=await mySqlPool.query(`INSERT INTO employee_info (Name,Email,Password,Role) VALUES (?,?,?,?)`,[obj.Name, obj.Email, obj.Password,obj.role]);
    // console.log(data);
    if(!data)
    {
      return   res.status(404).send({
    ...
    const Ctoken= jwt.sign({Name:obj.Name, Email:obj.Email,Role:tokenVerify.role},process.env.SECRET_KEY);
    obj.token=Ctoken;
    // console.log("This is Token")
    // console.log(token);
    // res.cookie('jwt', token, { httpOnly: true });
